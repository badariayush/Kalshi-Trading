...
class SignalWorkerTests(unittest.TestCase):
    def test_generates_informational_signal_feature_snapshot(self) -> None:
        ...
        self.assertEqual(signal.features.market_trend, "bullish")
        self.assertEqual(signal.features.structure_break, "bullish_bos")
        self.assertEqual(signal.features.vwap_proxy, Decimal("101300.0000000000000000000000"))
        self.assertGreater(signal.features.distance_from_vwap, Decimal("0"))
        self.assertIn("open_interest", signal.features.unavailable_context)
        self.assertIn("funding", signal.features.unavailable_context)
        self.assertIn("liquidation_map", signal.features.unavailable_context)
        self.assertIn("on_chain_metrics", signal.features.unavailable_context)
